...
impl FromStr for Addr {
    ...
    fn from_str(s: &str) -> Result<Self> {
        let url = s.parse::<Url>()?;
        match url.host() {
            Some(Host::Ipv4(addr)) => Ok(Addr::Ipv4(addr)),
            Some(Host::Ipv6(addr)) => Ok(Addr::Ipv6(addr)),
            Some(Host::Domain(domain)) => Ok(Addr::Domain(domain.to_string())),
            None => Err(Error::ParseHost),
        }
    }
}
...
mod tests {
    ...
    fn ipv4() {
        assert_eq!(
            Addr::from_str("http://127.0.0.1:5858").unwrap(),
    ...
    fn ipv6() {
        assert_eq!(
            Addr::from_str("http://[2001:0db8:11a3:09d7:1f34:8a2e:07a0:765d]:5858").unwrap(),
